...
import { generarExpediente, valoresTecnicos, calcular, distribuidoraPorCups } from "./motor.js?v=202609221234";
import { buscarCodigoPostal, claveCalle, codigosDe, esCodigoPostal, municipioDe,
         normalizar } from "./cp.js?v=202609221234";
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    CFG = await (await fetch("config-inicial.json?v=202609221234")).json();
  } catch (e) {
...
async function camposDelImpreso(archivo) {
  if (CAMPOS_IMPRESOS === null) {
    try {
      CAMPOS_IMPRESOS = await (await fetch("plantillas/campos.json?v=202609221234")).json();
    } catch (e) {
      CAMPOS_IMPRESOS = {};
    }
  }
